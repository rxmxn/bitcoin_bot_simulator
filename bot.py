...
class Config:
    def __init__(self,
        ...
        safety_order_size:float=20.0,
        target_profit_perc:float=1.0,
        price_deviation_safety_orders:float=0.5,
        safety_order_volume_scale:float=2.0,
        safety_order_step_scale:float=1.0) -> None:
    ...
    def max_safety_order_price_deviation(self) -> float:
        i = 1
...
class Bot:
    ...
    def buy(self, amount_usd: float, price: float, date: datetime) -> float:
    ...
    def sell(self, amount_btc: float, price: float, date: datetime) -> float:
    ...
    def calculate_gains_usd(self) -> float:
        return self.total_usd - self.initial_usd

    def calculate_deviation(self, last_price: float, initial_price: float) -> float:
        return 100 - (last_price * 100 / initial_price)

    def simulate(self, prices: list[float], dates: list[int], start_index: int):
        ...
        target_profit_perc = self.config.target_profit_perc
        ...
        for index, price in enumerate(prices):
            ...
            expected_profit = total_volume_usd * target_profit_perc / 100
            ...
            if price <= bought_price  * ( 1 - price_deviation_safety_orders / 100):
                if order_volume_usd <= self.total_usd:
                    ...
                    order_volume_usd *= safety_order_volume_scale
                    safety_trades_count += 1
                    price_deviation_safety_orders *= safety_order_step_scale
    ...
    def execute(self, prices: list[float], dates: list[int]):
        index = -1
